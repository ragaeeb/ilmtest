...
async function BooksGrid() {
    const res = await fetchWithCookies(`${process.env.NEXT_PUBLIC_BASE_URL ?? ''}/api/books`, {
        cache: 'force-cache',
    });
    if (!res.ok) {
    ...
    return (
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            {books.map((book: any) => (
                <Link
                    href={book.id === 'quran' ? '/books/quran' : `/books/${book.id}`}
                    key={book.id}
                ...
                >
                    ...
                    <div className="relative z-10">
                        <div className="mb-4 flex justify-center">
                            <Avatar src={book.avatar} alt={book.title} />
                        </div>
                        ...
                        {book.author?.name && (
                            <p className="mb-2 text-center text-sky-500 text-sm dark:text-sky-400">{book.author.name}</p>
                        )}
        ...
        <div className="min-h-screen bg-gradient-to-br from-sky-50 via-blue-50 to-indigo-100 dark:from-sky-950 dark:via-blue-950 dark:to-indigo-950">
            <div className="container mx-auto px-4 py-12">
                <div className="mb-16 text-center">
                    <img
                        src="https://pbs.twimg.com/profile_banners/876532308018331649/1563377978/1500x500"
                        alt="IlmTest banner"
                        className="mx-auto mb-6 h-32 w-full object-cover rounded"
                    />
                    ...
                    <h2 className="mb-4 font-semibold text-3xl text-sky-700 dark:text-sky-300">Islām In Its Original Form</h2>
                    <p className="mx-auto max-w-2xl text-sky-600 text-xl leading-relaxed dark:text-sky-400">
                        Your complete digital library for the Quran and authentic hadith collections. Explore the timeless wisdom of Islamic texts with beautiful translations.
                    </p>
                    ...
                    <>
                        <div className="mb-8 rounded-2xl border border-sky-200 bg-white/70 p-6 shadow-xl backdrop-blur-sm dark:border-sky-800 dark:bg-gray-900/70">
                            ...
                            <p className="text-sky-600 dark:text-sky-400">
                                Welcome back! Select a book below to continue your studies.
                            </p>
                        </div>
                        {/* Books Grid */}
                        <BooksGrid />
